...
import optimizers.MGWOO as mgwoo
import csv
import numpy
import time
    ...
    for j in range (0, len(datasets)):        # specfiy the number of the datasets
        for i in range (0, len(optimizer)):    
            for k in range (0,NumOfRuns):
                ...
                trainInput, testInput, trainOutput, testOutput = train_test_split(X, y, test_size=0.10, random_state=34809)
                
                numFeatures=numpy.shape(trainInput)[1]#number of features in the train dataset

                #number of hidden neurons
                HiddenNeurons = numFeatures*4+2
                net = nl.net.newff([[0, 1]]*numFeatures, [int(HiddenNeurons/2),int(HiddenNeurons/2), 1])
                
                dim=int((numFeatures*HiddenNeurons/2)+((HiddenNeurons/2)*(HiddenNeurons/2))+((3*HiddenNeurons)/2)+1);
...
def selector(algo, func_details, dim, popSize, Iter, trainInput,trainOutput,net):
    ...
    if(algo=="PSO"):
        ...
        x=mgwo.MGWO(getattr(costNN, function_name),lb,ub,dim,popSize,Iter,trainInput,trainOutput,net)
    elif(algo=="MGWOO"):
        x=mgwoo.MGWO(getattr(costNN, function_name),lb,ub,dim,popSize,Iter,trainInput,trainOutput,net)
